...
def load_openml_data(task_id):
    ...
    y_train = np.asarray(pd.to_numeric(y.iloc[train_indices], errors="coerce"), dtype=np.float64)
    y_test = np.asarray(pd.to_numeric(y.iloc[test_indices], errors="coerce"), dtype=np.float64)

    print(f"Training set shape: {X_train.shape}")
    print(f"Test set shape: {X_test.shape}")
    print(f"Number of features: {X_train.shape[1]}")

    return X_train, y_train, X_test, y_test, attribute_names
...
def train_spex_ebm_models(X_train, y_train, X_test, y_test, manual_interactions_set, index_name):
...
def main():
    ...
    parser.add_argument('--interaction-dir', type=str, default='./interaction_12_14_2025',
                        help='Directory containing interaction pickle files (default: ./interaction_12_14_2025)')
    ...
    print("\nLoading OpenML data...")
    X_train, y_train, X_test, y_test, attribute_names = load_openml_data(args.task_id)
    ...
    for index in available_indices:
        try:
            ...
            ebm_models, results = train_spex_ebm_models(
                X_train, y_train, X_test, y_test, manual_interactions_set, index
            )
