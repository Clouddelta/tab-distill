...
try:
    ...
    sys.path.insert(0, str(BASE_DIR / 'spectral-explain' / 'src'))
    import spectralexplain as spex
...
def process_task(
    ...
    num_samples=2,
    index_types=None,
    output_dir=str(BASE_DIR / 'interaction_1_14_2026_500')
):
    """
    ...
    num_samples_to_process = min(num_samples_to_use, 500)
    ...
    sample_slice = train_set[:num_samples_to_process]

    for idx, train_point in enumerate(sample_slice):
        print(f"\nProcessing training sample {idx + 1}/{len(sample_slice)}...")
        # if idx ==3:
        #     break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run TabDistill interaction search for a single OpenML task"
    )
    parser.add_argument("task_id", type=int, help="OpenML task ID")
    parser.add_argument(
        "--num-samples", type=int, default=0,
        help="Number of training samples to process (0 = all, capped at 500)",
    )
    parser.add_argument(
        "--output-dir", type=str,
        default=str(BASE_DIR / "interaction_output"),
        help="Output directory for interaction pickle files",
    )
    args = parser.parse_args()
    process_task(args.task_id, num_samples=args.num_samples, output_dir=args.output_dir)